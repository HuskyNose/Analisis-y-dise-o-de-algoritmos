...
export function renderGraph() {
  if (window.graphLayer) {
    window.graphLayer.clearLayers();
  } else {
  ...
  if (graph.edges) {
    graph.edges.forEach(edge => {
      ...
      if (sourceNode && targetNode) {
        const latlngs = [
          [parseFloat(sourceNode.lat), parseFloat(sourceNode.lng)],
          [parseFloat(targetNode.lat), parseFloat(targetNode.lng)]
        ];
        ...
        polyline.bindTooltip(String(edge.distance), { 
          permanent: true, 
          direction: 'center',
